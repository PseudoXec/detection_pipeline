...
def _redact_url(url: str) -> str:
    """Strip user:pass@ credentials out of an RTSP/HTTP URL before it ever
    reaches a print(), a log line, or an exception message - those all end
    up on disk (console redirect, journald, systemd's crash logs) in plain
    text, so the URL you put in config.yaml/secrets.yaml would otherwise
    leak straight back out through the logs even after you lock the config
    file itself down.
    """
    try:
        parts = urlsplit(url)
        ...
        # if the URL is malformed in some way we don't recognize, fail safe
        # rather than risk printing it unredacted
        return "<redacted>"
...
class ThreadedRTSPCamera:
    ...
    def start(self) -> "ThreadedRTSPCamera":
        ...
        try:
            ...
            print(f"[camera] connected to {_redact_url(self.rtsp_url)}")
    ...
    def _open_capture(self) -> None:
        ...
        if not capture.isOpened():
            capture.release()
            raise RuntimeError(f"Could not open RTSP stream: {_redact_url(self.rtsp_url)}")
