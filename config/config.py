...
class OcrConfig:
    ...
    # kept for config compatibility; FastPlateOCR's model zoo isn't language-keyed
    lang: str = "en"
    # FastPlateOCR model name (or a path to your own exported ONNX model) that
    # PlateOCRReader loads - see the FastPlateOCR model zoo for alternatives
    ...
    # FastPlateOCR's ONNX inference is lightweight, but 1 is usually still right for a
    # Pi 5 (leaves cores for the vehicle/plate models + RTSP decode + live view); raise
    # to 2 only if the finalize queue keeps backing up (see the "[finalize] queue full"
    # log line) AND model.inference_threads leaves enough cores spare.
...
class FeaturesConfig:
    ...
    async_ocr: bool = True                       # run OCR + record-build/store on background thread(s) instead of
                                                  ...
                                                  # FastPlateOCR's latency; see pipeline/finalize_worker.py. The plate
...
class PipelineConfig:
    ...
    def load(cls, yaml_path: Optional[str] = None) -> "PipelineConfig":
        """Build a config in three layers, each overriding the last:

            1. defaults (below)
            2. config.yaml               - day-to-day tuning, safe to share/commit
            3. secrets.yaml (or $PIPELINE_SECRETS_FILE) - credentials only,
               kept out of config.yaml entirely so the file you'd normally
               screenshot, zip up for a bug report, or accidentally `git add`
               never has a password or token in it. Not required to exist;
               the pipeline still runs off config.yaml alone (with secrets
               left null) if you haven't split it out.
            4. environment variables - PIPELINE_<SECTION>_<FIELD>, e.g.
               PIPELINE_CAMERA_RTSP_URL. Highest precedence, useful for a
               systemd EnvironmentFile= (see detection-pipeline.service)
               instead of a secrets file on disk at all.
        """
        ...
        if yaml_path and os.path.isfile(yaml_path):
            ...
            cls._apply_dict(config, raw)

        secrets_path = os.environ.get("PIPELINE_SECRETS_FILE") or str(CONFIG_DIR / "secrets.yaml")
        if os.path.isfile(secrets_path):
            _warn_if_not_locked_down(secrets_path)
            with open(secrets_path, "r", encoding="utf-8") as handle:
                secret_raw = yaml.safe_load(handle) or {}
            cls._apply_dict(config, secret_raw)
    ...
    @staticmethod
    def _apply_dict(config: "PipelineConfig", raw: dict) -> None:
        """Walk each top-level section (camera, roi, model, ...) and overwrite
        only the fields actually present, leaving the rest untouched."""
    ...
    def _apply_env(config: "PipelineConfig") -> None:
        """Override any field from an env var named PIPELINE_<SECTION>_<FIELD>
        (case-insensitive on the env var name convention, matched uppercase).
        Value is coerced to match the type of whatever is already set, so
        booleans/ints/floats keep working, not just strings."""
        for section_name in (
            "camera", "roi", "model", "tracking", "crop", "preprocess",
            "ocr", "storage", "runtime", "features", "api", "live",
        ):
            section_obj = getattr(config, section_name)
            for key in vars(section_obj):
                env_name = f"PIPELINE_{section_name.upper()}_{key.upper()}"
                if env_name not in os.environ:
                    continue
                raw_value = os.environ[env_name]
                setattr(section_obj, key, _coerce(raw_value, getattr(section_obj, key)))

    def as_dict(self) -> dict:
        """Flat dict view, mostly useful for debug logging at startup.

        NOTE: this intentionally is NOT redacted - it's meant for local
        debugging only. Never log/print the result of this at INFO level or
        above in a 24/7 service; use as_dict_redacted() for anything that
        might reach journald.
        """
        return asdict(self)

    def as_dict_redacted(self) -> dict:
        """Same as as_dict(), but with known-sensitive fields masked. Safe to
        log at startup so you can confirm what loaded without leaking it."""
        data = asdict(self)
        if data.get("camera", {}).get("rtsp_url"):
            data["camera"]["rtsp_url"] = _redact_secret_url(data["camera"]["rtsp_url"])
        if data.get("live", {}).get("auth_token"):
            data["live"]["auth_token"] = "***"
        if data.get("live", {}).get("endpoint_url"):
            data["live"]["endpoint_url"] = "***"
        if data.get("api", {}).get("endpoint_url"):
            data["api"]["endpoint_url"] = "***"
        return data


def _coerce(raw_value: str, current):
    """Cast an environment-variable string to match the type of the existing
    config value (so PIPELINE_MODEL_INFERENCE_THREADS=4 becomes an int, etc.)."""
    if current is None:
        return raw_value
...
def _redact_secret_url(url: str) -> str:
    """Best-effort user:pass@ redaction for display/logging, independent of
    camera.py's copy so config.py has no import dependency on it."""
    if "@" not in url:
        return url
    scheme_sep = url.find("://")
    if scheme_sep == -1:
...
def _warn_if_not_locked_down(path: str) -> None:
    """secrets.yaml holds plaintext credentials - warn loudly (but don't
    crash) if its permissions let anyone else on the Pi read it. This never
    runs for config.yaml, which is expected to stay shareable."""
    try:
    ...
    if mode & (stat.S_IRWXG | stat.S_IRWXO):
        warnings.warn(
            f"{path} is readable by users other than its owner (mode {oct(mode)}). "
