...
class TrackingConfig:
    ...
    # with max_plate_attempts > 1 the pipeline keeps the BEST plate seen so far (by
    # plate-detector confidence) and only stops early once that confidence clears
    # model.plate_conf_threshold. Retries are spaced this far apart so each one looks
    # at a genuinely different frame - at 10 fps, consecutive frames are near-identical.
...
class StorageConfig:
    ...
    # (this is the image the API/C# side receives; any plate-number reading
    # happens entirely on their end, not in this pipeline)
...
class PipelineConfig:
    ...
    def _apply_env(config: "PipelineConfig") -> None:
        ...
        for section_name in (
            ...
            "storage", "runtime", "features", "api", "live",
        ):
            section_obj = getattr(config, section_name)
