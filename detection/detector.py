...
detector.py
-----------
Thin wrapper around Ultralytics YOLO: loading a model (.pt or an exported
NCNN folder), running detection on a single image, running detection on
...
def weights_exist(path: str) -> bool:
    """True if `path` points at a .pt file or an NCNN export folder."""
...
def load_model(weights_path: str, device: Optional[str] = None) -> YOLO:
    """Load a YOLO model, whether it's a raw .pt checkpoint or an NCNN export.
...
def detect_batch(
    ...
    # A static-shape NCNN export is compiled for exactly one input shape,
    # e.g. [1, 3, 640, 640] - batch size baked in as 1. Handing it more than
    # one image at once then fails
