...
API" flow described in the project brief lives. Every other module is a
building block; this module wires them together for one frame at a time.

No text recognition (OCR) happens anywhere in this pipeline: the plate crop
image itself is what gets stored/sent, and any reading of the plate number
is left entirely to whatever receives it on the other end of the API.
...
Flow for a single frame:
    ...
    6. Store           - hand the finished record (metadata + both images +
                         per-vehicle timing) to the SQLite buffer, which
                         forwards it to the API when features.send_via_api is on
"""
...
class _PlateCandidate:
    """One plate sighting: the crop and where it was found. A vehicle keeps only its
    best candidate across attempts (by plate detector confidence), and that one is
    ...
    raw_crop: Optional[np.ndarray] = None    # un-enhanced plate crop, when different from plate_crop

    @property
    def rank(self) -> float:
        """Higher is better: plate detector confidence decides which sighting wins."""
        return self.plate_conf
...
class DetectionPipeline:
    ...
    def _evaluate_plate(self, state: _TrackState, plate_predictions: List[Dict[str, Any]]) -> Optional[_PlateCandidate]:
        """Cut the best plate box out of this attempt's vehicle crop and enhance it.
    ...
    def _is_good_enough(self, candidate: Optional[_PlateCandidate]) -> bool:
        """True when there's no point trying another frame for this vehicle: the
        plate box is picked by plate-detector confidence alone (no OCR score to
        judge readability by), same fallback this pipeline always used when no
        OCR engine was available."""
    ...
    def _finalize_vehicle(self, track_id: str, state: _TrackState) -> None:
        ...
        crops. With no OCR pass to wait on, building the record is cheap (JPEG encode
        + box math) and always happens right here, inline in the detection loop."""
        best = state.best
    ...
    def _build_record(
        ...
        """Builds the finished DetectionRecord for one vehicle: encodes the JPEGs and
        assembles every column. No OCR runs anywhere here - the plate crop image
        itself is the payload; any failure must never raise past this function."""
        features = self.config.features
    ...
    def process_frame(self, frame: np.ndarray, always_finalize: bool = False, captured_at: Optional[float] = None) -> int:
        ...
        if not features.plate_detection:
            ...
            for track_id, plate_predictions in plate_results.items():
                ...
                # cut + enhance this sighting, keep it only if it beats what we have
