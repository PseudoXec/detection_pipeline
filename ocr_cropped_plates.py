...
def main():
    ...
    for original_filename in image_files:
        ...
        if not variants:
            print(f"{original_filename} -> check (unreadable image)")
            continue
        ...
        if not args.no_rename:
            ...
            # Keeps the crop's full name (which already carries the source
            # photo's filename) and tags on _read_/_check_ + the recognized
            # text, so the OCR result can be traced back to both the crop
            # and the original photo just from the filename.
            base_name = f"{stem}_{status}_{sanitized_text}"
            new_path = unique_destination(args.input_dir, base_name, ext)
            os.rename(image_path, new_path)
            new_filename = os.path.basename(new_path)

        # Short status line: full detail (confidence, variant, rename,
        # low-confidence flag) lives in the CSV, not the console.
        detail = text if text else "no text found"
        print(f"{original_filename} -> {status} ({detail})")
        rows.append({
