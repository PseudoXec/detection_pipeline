...
| `export_ncnn.py` | One-off utility: convert a `.pt` model to a faster NCNN export |
...
Edit `config/config.yaml` (see the comments inside it for every option) for day-to-day
tuning - this file is safe to share, screenshot, or commit.

Credentials and internal URLs (camera RTSP URL, live-view auth token, the dashboard API
endpoint) do **not** go in `config.yaml`. They go in `config/secrets.yaml`, which is
gitignored and should be `chmod 600`:

```bash
cp config/secrets.yaml.example config/secrets.yaml
nano config/secrets.yaml        # fill in the real rtsp_url, auth_token, endpoint_url
chmod 600 config/secrets.yaml
```

Running `sudo ./secure_setup.sh` (see below) does the ownership/permissions part of this
for you and creates the dedicated `pipeline` service account. See `SECURITY.md` for the
full rundown of what changed and why.
...
`export_ncnn.py` converts a `.pt` checkpoint into an NCNN export, which runs
noticeably faster than `.pt`/`.onnx` on the Pi 5's ARM CPU:
```bash
...
Ultralytics' own Pi 5 benchmark shows the opposite, which is why this project
doesn't use it. Prefer `.onnx` (the current default) or NCNN there.
