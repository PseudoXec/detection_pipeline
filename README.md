...
noticeably faster than ONNX Runtime on the Pi 5's ARM CPU (Ultralytics' own
Pi 5 benchmarks agree):
```bash
...
error. The vehicle model is exported as a **rectangle**, `[height, width]`:

```python
from ultralytics import YOLO
YOLO("models/vehicle.pt").export(format="ncnn", imgsz=(512, 896))   # (height, width)
```
